...
class TrackerRankNetEstimator(RegressorMixin, BaseEstimator):
    ...
    def __init__(
        ...
        self.random_state = random_state
        # None auto-detects; pass an explicit torch device string to pin one.
        self.device = device
    ...
    with skrub.config_context(eager_data_ops=False):
        ...
        pred = features.skb.apply(
            TrackerRankNetEstimator(
                ...
                # Pinned, not auto-detected. `torch.backends.mps.is_available()`
                # is True on the macOS CI runner, but its virtualised Metal
                # device cannot back an allocation, so the first transfer dies
                # with an out-of-memory error reporting 0 bytes allocated and
                # 7.93 GiB free. A model this size has nothing to gain from a
                # GPU anyway, and CPU keeps the run reproducible across jobs.
                device="cpu",
            ),
            y=y,
            fit_kwargs={
